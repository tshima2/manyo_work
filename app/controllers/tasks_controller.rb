  ...
  def index
    ...
    if @filter_name
      if @filter_status
        ...
        @tasks=Task.name_like(@filter_name).status_search(@filter_status).id_sort
      else
        # @tasks=Task.where('name LIKE ?', "%#{@filter_name}%").order(id: 'ASC')
        @tasks=Task.name_like(@filter_name).id_sort
      end

    else
      if @filter_status
        # @tasks=Task.where('status = ?', @filter_status).order(id: 'ASC')
        @tasks=Task.status_search(@filter_status).id_sort
      else
        # ソート用リンクが踏まれた場合
